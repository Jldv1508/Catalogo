...
const OWNER_LOGIN_TOKEN_MAX_AGE_SECONDS = 60 * 20;
...
export async function createOwnerLoginToken(payload) {
  return createSignedToken({
    ...payload,
    type: 'owner-login',
    exp: Date.now() + (OWNER_LOGIN_TOKEN_MAX_AGE_SECONDS * 1000),
  });
}

export async function verifyOwnerLoginToken(token) {
  const payload = await verifySignedToken(token);
  return payload?.type === 'owner-login' ? payload : null;
}
